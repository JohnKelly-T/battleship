...
describe("Game class", () => {
  ...
  describe("toggleTurn method", () => {
    test("swaps current turn player and enemy", () => {
      expect(game.turnPlayer).toMatchObject(game.player1);

      game.toggleTurn();

      expect(game.turnPlayer).toMatchObject(game.player2);
    });
  });
  ...
  describe("makeMove method", () => {
    test("makes move on correct player", () => {
      ...
      expect(player2.gameboard.receivedAttacks[yMove][xMove]).not.toBeNull();
      ...
      expect(player1.gameboard.receivedAttacks[yMove][xMove]).not.toBeNull();
    });
  });
